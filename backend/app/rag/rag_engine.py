...
Remember: Answer ONLY based on the evidence above. Cite specific PR numbers. If evidence is insufficient, say so."""
    ...
    async def ask_stream(
        ...
        *,
        ...
        """
        Process a question through the RAG pipeline with real-time token streaming.

        Yields dict events:
        - "evidence": initial retrieved evidence documents & retrieval latency
        - "token": each text token generated by the LLM
        - "done": final response object with evidence tracking & latency metrics
        """
        latency: dict[str, float] = {}
        total_start = time.monotonic()
        ...
        try:
            ...
            logger.error("Retrieval failed in stream: %s", e)
            yield {
                "type": "error",
        ...
        # Yield initial evidence event to frontend
        yield {
            "type": "evidence",
            "evidence": evidence_summary,
            "latency": latency,
...
Remember: Answer ONLY based on the evidence above. Cite specific PR numbers. If evidence is insufficient, say so."""
        ...
        retrieved_pr_numbers = {r.pr_number for r in results if r.pr_number}
        tracking = self.evidence_tracker.track_evidence(
            full_answer, retrieved_pr_numbers, evidence_summary
        )
